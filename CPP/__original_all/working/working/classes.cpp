...
void parameters_class::recompute()
{
  ...
  fprintf(logg,"apart from spatial %i degree(s) of freedom considered: ",dof_prototype::end);
  if (dof_prototype::spin<dof_prototype::end) fprintf(logg,"spin ");
  if (dof_prototype::sigma<dof_prototype::end) fprintf(logg,"sigma ");
  //if (dof_prototype::valley<dof_prototype::end) fprintf(logg,"valley ");
  //if (dof_prototype::kappa<dof_prototype::end) fprintf(logg,"kappa ");
  fprintf(logg,". The system tag is '%c' and number of components is %i.\n",system, (int) parameters.read("sets","-"));
  if (system=='A') if (dof_prototype::sigma<dof_prototype::end) {printf("sigma (%i vs end=%i) not defined in GaAs [in recompute()]: edit classes.h dof_prototype\n", dof_prototype::sigma, dof_prototype::end);exit(1);}
...
void state_info::compute(int u, int t)
{
  ...
  switch (t) {
    case 0: {//sx
      if (dof_prototype::spin>dof_prototype::end) x[u][0]=cos(phi_B)*sin(theta_B);
      else {
	//PREVIOUS
    ...
    case 1: {//sy
      if (dof_prototype::spin>dof_prototype::end) x[u][1]=sin(phi_B)*sin(theta_B);
      else {
	//PREVIOUS
    ...
    case 2: {//sz
      if (dof_prototype::spin>dof_prototype::end) x[u][2]=cos(theta_B);
      else {
	//PREVIOUS
    ...
    case 27: {//sigmaz - graphene: sigmaz
      if (dof_prototype::sigma>dof_prototype::end) {
...
dof_prototype::dof_prototype() 			//constructor
{
  Nc=end;
  length=1;
  for (int i=0;i<Nc;i++) length*=2;
  indexC=new int[Nc];
  operatorC=new int[Nc];
...
void dof_prototype::reset(const char* what)
{
  ...
  switch (choice) {
    case 0 : {for (int i=0;i<Nc;i++) operatorC[i]=0;break;}
    case 1 : {for (int i=0;i<length*length;i++) M_sum[i]=0;break;}
    case 2 : {for (int i=0;i<Nc;i++) indexC[i]=1;break;}
...
void dof_prototype::convert_log(const char* mess, const char* what, complex<double> c)
{
  ...
  for (int i=0; i<Nc; i++) fprintf(logg,"indexC[%i]=%i ",i,indexC[i]);
  fprintf(logg,"index=%i\n",index);
  for (int i=0; i<Nc; i++) fprintf(logg,"operatorC[%i]=%i ",i,operatorC[i]);
  fprintf(logg,"\n");
...
void dof_prototype::convert(const char* what, complex<double> coef)
{
  ...
  switch (choice) {
    case 0 : {
      index=0;
      for (int i=0, p=1;i<Nc;i++, p*=2) if (indexC[i]==-1) index+=p; else if (indexC[i]!=1) {printf("component index(%i)=%i out of scope in dof_prototype::convert\n",i,indexC[i]); exit(1);}
      break;
    }
    case 1 : {
      if (index<0 || index>=length) {printf("consequtive index %i out of scope(0,%i) in dof_vector_prototype::setI\n",index,length); exit(1);}
      for (int i=0;i<Nc;i++,index/=2) if (index % 2) indexC[i]=-1; else indexC[i]=1;
      break;
    }
    case 2 : {
      if (Nc<1) {printf("Nc=%i in dof_prototype::convert\n",Nc);exit(1);}
      for (int i=0;i<length*length;i++) M[i]=0;
      for (int i=0;i<Nc;i++) pntr[i]=0;
      do {
        complex<double> c=1;
        for (int i=0;i<Nc;i++) c*=sigma2ketbra(operatorC[i],pntr[i],ketC[i],braC[i]);
	      int ket=0, bra=0;
        for (int i=0, p=1;i<Nc;i++, p*=2) {
          if (ketC[i]==-1) ket+=p;
          if (braC[i]==-1) bra+=p;
        }
        M[ket*length+bra]=c;
        pntr[0]++;
        for (int i=0;i<Nc-1;i++) if (pntr[i]==2) {pntr[i]=0;pntr[i+1]++;}
      } while (pntr[Nc-1]<2);
