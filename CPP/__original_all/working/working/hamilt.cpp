...
void hamiltonian_init(reg1& region)
{
  ...
  //operators diagonal in spin/valley indexes
  //!potential
  ...
  //!spin impurities
  fprintf(logg,"Ham check: before spin impurities\n");
  prec Arho=-parameters.read("exch","meV")*parameters.read("xMn","-")*(5.0/2)*(1.0/2);
  if (Arho!=0 && spin_impurities.N!=0) {
    if (sets==1) {
      printf("spin impurities are present, but the particle has no spin (sets=%i)\n",sets);
      exit(1);
    }
    fprintf(logg,"Ham check: spin impurities\n");
    dof.reset("indexes");
    for (int i=0;i<2;i++) for (int j=0;j<2;j++) {
      ...
      dof.convert("C2I");
      int J=dof.index;
      //region.operate_tot_init(i,j,reg1::lin_com, content(1,0), spin_potential, exp_integral); PREVIOUS
      region.operate_tot_init(I,J,reg1::lin_com, content(1,0), spin_potential, exp_integral);
      //fprintf(logg,"(%i,%i) old vs dof (%i,%i)\n",i,j,I,J);
    }
  }
  ...
  if (sets>1) {
    ...
    /*fprintf(logg,"Ham check: Zeeman\n");
    fprintf(logg,"(0,0) old %e%+ei vs dof %e%+ei (res %e)\n",Bz*gz*u.real(), Bz*gz*u.imag(), dof.M_sum[0*dof.length+0].real(), dof.M_sum[0*dof.length+0].imag(), abs(Bz*gz*u-dof.M_sum[0*dof.length+0]));
    fprintf(logg,"(0,1) old %e%+ei vs dof %e%+ei (res %e)\n",(content(gx*Bx,gy*By)*u).real(), (content(gx*Bx,gy*By)*u).imag(), dof.M_sum[1*dof.length+0].real(), dof.M_sum[1*dof.length+0].imag(), abs(content(gx*Bx,gy*By)*u-dof.M_sum[1*dof.length+0]));
    fprintf(logg,"(1,0) old %e%+ei vs dof %e%+ei (res %e)\n",(content(gx*Bx,-gy*By)*u).real(), (content(gx*Bx,-gy*By)*u).imag(), dof.M_sum[0*dof.length+1].real(), dof.M_sum[0*dof.length+1].imag(), abs(content(gx*Bx,-gy*By)*u-dof.M_sum[0*dof.length+1]));
    fprintf(logg,"(1,1) old %e%+ei vs dof %e%+ei (res %e)\n",-Bz*gz*u.real(), -Bz*gz*u.imag(), dof.M_sum[1*dof.length+1].real(), dof.M_sum[1*dof.length+1].imag(), abs(-Bz*gz*u-dof.M_sum[1*dof.length+1]));*/
  }
  
  prec br=parameters.read("br","meVA")*units.meV*1e-10;
  prec dress=parameters.read("dress","meVA")*units.meV*1e-10;
  prec dress3=parameters.read("dress3","eVA^3")*units.e*1e-30;
  if (geometry!=1 && sets==2) {//2D problem
    if (br!=0 || dress!=0) {
      //Bytchkov-Rashba + Dresselhaus
      u=1/units.length/units.energy;
      if ((int) growth==1) {
	//PREVIOUS
        //printf("strength of br in Hamiltonian (meV):%e\n",br*units.energy/(1e-3*units.e));
        //the original spatially uniform spin-orbit interaction
        //region.operate_tot_init(1,0,reg1::dy, content(-dress,-br)*u,0,exp_integral);
        //region.operate_tot_init(1,0,reg1::dx, content(br,dress)*u,0,exp_integral);
        //region.operate_tot_init(0,1,reg1::dy, content(dress,-br)*u,0,exp_integral);
        //region.operate_tot_init(0,1,reg1::dx, content(-br,dress)*u,0,exp_integral);
	...
	/*fprintf(logg,"Ham check: soc:dx\n");
	fprintf(logg,"(0,0) old %e%+ei vs dof %e%+ei (res %e)\n",0.0, 0.0, dof.M_sum[0*dof.length+0].real(), dof.M_sum[0*dof.length+0].imag(), abs(0.0-dof.M_sum[0*dof.length+0]));
	fprintf(logg,"(0,1) old %e%+ei vs dof %e%+ei (res %e)\n",(content(-br,dress)*u).real(), (content(-br,dress)*u).imag(), dof.M_sum[1*dof.length+0].real(), dof.M_sum[1*dof.length+0].imag(), abs(content(-br,dress)*u-dof.M_sum[1*dof.length+0]));
	fprintf(logg,"(1,0) old %e%+ei vs dof %e%+ei (res %e)\n",(content(br,dress)*u).real(), ( content(br,dress)*u).imag(), dof.M_sum[0*dof.length+1].real(), dof.M_sum[0*dof.length+1].imag(), abs(content(br,dress)*u-dof.M_sum[0*dof.length+1]));
	fprintf(logg,"(1,1) old %e%+ei vs dof %e%+ei (res %e)\n",0.0, 0.0, dof.M_sum[1*dof.length+1].real(), dof.M_sum[1*dof.length+1].imag(), abs(0.0-dof.M_sum[1*dof.length+1]));*/
	for (int i=0;i<dof.length;i++) for (int j=0;j<dof.length;j++) 
	...
	dof.operatorC[dof_prototype::spin]=dof_prototype::pauliy;
	dof.convert("OP2M",dress*content(0,-1)*u);
	/*fprintf(logg,"Ham check: soc:dy\n");
	fprintf(logg,"(0,0) old %e%+ei vs dof %e%+ei (res %e)\n",0.0, 0.0, dof.M_sum[0*dof.length+0].real(), dof.M_sum[0*dof.length+0].imag(), abs(0.0-dof.M_sum[0*dof.length+0]));
	fprintf(logg,"(0,1) old %e%+ei vs dof %e%+ei (res %e)\n",(content(dress,-br)*u).real(), (content(dress,-br)*u).imag(), dof.M_sum[1*dof.length+0].real(), dof.M_sum[1*dof.length+0].imag(), abs(content(dress,-br)*u-dof.M_sum[1*dof.length+0]));
	fprintf(logg,"(1,0) old %e%+ei vs dof %e%+ei (res %e)\n",(content(-dress,-br)*u).real(), (content(-dress,-br)*u).imag(), dof.M_sum[0*dof.length+1].real(), dof.M_sum[0*dof.length+1].imag(), abs(content(-dress,-br)*u-dof.M_sum[0*dof.length+1]));
	fprintf(logg,"(1,1) old %e%+ei vs dof %e%+ei (res %e)\n",0.0, 0.0, dof.M_sum[1*dof.length+1].real(), dof.M_sum[1*dof.length+1].imag(), abs(0.0-dof.M_sum[1*dof.length+1]));*/
	for (int i=0;i<dof.length;i++) for (int j=0;j<dof.length;j++) 
      ...
      if ((int) growth==110) {
	if (sets!=2) {fprintf(logg,"growth direction 110 not implemented for dof class and linear SOC\n");exit(1);}
    ...
    if (dress3!=0) { //Dresselhaus cubic
      ...
      if ((int) growth==1) {
	//PREVIOUS
        /*region.operate_tot_init(1,0,reg1::dxdydy, content(0,dress3)*u,0,exp_integral);
        region.operate_tot_init(1,0,reg1::dxdxdy, content(-dress3,0)*u,0,exp_integral);
        region.operate_tot_init(0,1,reg1::dxdydy, content(0,dress3)*u,0,exp_integral);
        region.operate_tot_init(0,1,reg1::dxdxdy, content(dress3,0)*u,0,exp_integral);*/
	...
	/*
	fprintf(logg,"Ham check: soc:dxdydy\n");
	fprintf(logg,"(0,0) old %e%+ei vs dof %e%+ei (res %e)\n",0.0, 0.0, dof.M_sum[0*dof.length+0].real(), dof.M_sum[0*dof.length+0].imag(), abs(0.0-dof.M_sum[0*dof.length+0]));
	fprintf(logg,"(0,1) old %e%+ei vs dof %e%+ei (res %e)\n",(content(0,dress3)*u).real(), (content(0,dress3)*u).imag(), dof.M_sum[1*dof.length+0].real(), dof.M_sum[1*dof.length+0].imag(), abs(content(0,dress3)*u-dof.M_sum[1*dof.length+0]));
	fprintf(logg,"(1,0) old %e%+ei vs dof %e%+ei (res %e)\n",(content(0,dress3)*u).real(), ( content(0,dress3)*u).imag(), dof.M_sum[0*dof.length+1].real(), dof.M_sum[0*dof.length+1].imag(), abs(content(0,dress3)*u-dof.M_sum[0*dof.length+1]));
	fprintf(logg,"(1,1) old %e%+ei vs dof %e%+ei (res %e)\n",0.0, 0.0, dof.M_sum[1*dof.length+1].real(), dof.M_sum[1*dof.length+1].imag(), abs(0.0-dof.M_sum[1*dof.length+1]));*/
	...
	/*fprintf(logg,"Ham check: soc:dxdxdy\n");
	fprintf(logg,"(0,0) old %e%+ei vs dof %e%+ei (res %e)\n",0.0, 0.0, dof.M_sum[0*dof.length+0].real(), dof.M_sum[0*dof.length+0].imag(), abs(0.0-dof.M_sum[0*dof.length+0]));
	fprintf(logg,"(0,1) old %e%+ei vs dof %e%+ei (res %e)\n",(content(dress3,0)*u).real(), (content(dress3,0)*u).imag(), dof.M_sum[1*dof.length+0].real(), dof.M_sum[1*dof.length+0].imag(), abs(content(dress3,0)*u-dof.M_sum[1*dof.length+0]));
	fprintf(logg,"(1,0) old %e%+ei vs dof %e%+ei (res %e)\n",(content(-dress3,0)*u).real(), ( content(-dress3,0)*u).imag(), dof.M_sum[0*dof.length+1].real(), dof.M_sum[0*dof.length+1].imag(), abs(content(-dress3,0)*u-dof.M_sum[0*dof.length+1]));
	fprintf(logg,"(1,1) old %e%+ei vs dof %e%+ei (res %e)\n",0.0, 0.0, dof.M_sum[1*dof.length+1].real(), dof.M_sum[1*dof.length+1].imag(), abs(0.0-dof.M_sum[1*dof.length+1]));*/

      }
      
      if ((int) growth==110) {
	if (sets!=2) {fprintf(logg,"growth direction 110 not implemented for dof class and cubic SOC\n");exit(1);}
  ...
  if (geometry==1 && sets==2) //1D ring
  {
    ...
    if (br!=0 || dress!=0) {
      //Bytchkov-Rashba + Dresselhaus
      if ((int) growth==1) {
	if (sets!=2) {fprintf(logg,"linear SOC not implemented for 1D ring and dof class\n");exit(1);}
        ...
        ring_eiphi_exp=-1;
        u=dress*content(0,1)/(2.0*ring)/units.energy;
	...
	if (sets!=2) {fprintf(logg,"linear SOC not implemented for 1D ring and dof class\n");exit(1);}
        ...
        u=dress3*content(0,1)/(16.0*ring*lv*lv);
        ring_eiphi_exp=1;
        region.operate_tot_init(0,1,reg1::dx,content(0,-ring)/units.length*u,ring_eiphi,exp_integral);
        region.operate_tot_init(0,1,reg1::lin_com,u/2.0,ring_eiphi,exp_integral);
        ring_eiphi_exp=-1;
...
/*
void hamiltonian1_init(reg1& region)
{
  region.act_prec_set(op_prec);
  region.symmetrize_ops(op_sym);
  for (int i=0;i<reg1::all_op;i++) region.remember_set((reg1::operators) i,op_rmmbr[i]);
}
...
void hamiltonian1(int N, void *A, content *x, content *y)
{
  ...
  //Psi'_0=H_00 Psi_0
  //pr->operate(x,y,0,0,reg1::dxdx,content(-1*0,0),0,reg1::set);
  pr->operate(x,y,0,0,reg1::dxdx,content(-1,0),0,reg1::set);
  pr->operate(x,y,0,0,reg1::dydy,content(-1,0),0,reg1::add);
  pr->operate(x,y,0,0,reg1::dx,content(0,-2),vektorcx,reg1::add);
  pr->operate(x,y,0,0,reg1::dy,content(0,-2),vektorcy,reg1::add);
  pr->operate(x,y,0,0,reg1::lin_com,content(1,0),h_00,reg1::add);

  //Psi'_0=H_01 Psi_1
  if ((ab!=0) || (ad!=0)) {
  pr->operate(x,y,1,0,reg1::dx,content(ab,ad),0,reg1::add);
  pr->operate(x,y,1,0,reg1::dy,content(-ad,-ab),0,reg1::add);
  pr->operate(x,y,1,0,reg1::lin_com,content(1,0),h_01,reg1::add);
  }

  //H_D3_01

  if (ad3!=0) {
  pr->operate(x,y,1,0,reg1::dxdxdy,content(-2*ad3,0),0,reg1::add);
  pr->operate(x,y,1,0,reg1::dxdydy,content(0,2*ad3),0,reg1::add);

  pr->operate(x,y,1,0,reg1::dxdx,content(0,-2*ad3),vektorcy,reg1::add);
  pr->operate(x,y,1,0,reg1::dydy,content(-2*ad3,0),vektorcx,reg1::add);
  pr->operate(x,y,1,0,reg1::dxdy,content(ad3,0),hd3_01_1,reg1::add);

  pr->operate(x,y,1,0,reg1::dx,content(ad3,0),hd3_01_2,reg1::add);
  pr->operate(x,y,1,0,reg1::dy,content(ad3,0),hd3_01_3,reg1::add);

  pr->operate(x,y,1,0,reg1::lin_com,content(ad3,0),hd3_01_4,reg1::add);

  }

  //Psi'_1=H_11 Psi_1
  //pr->operate(x,y,1,1,reg1::dxdx,content(-1*0,0),0,reg1::set);
  pr->operate(x,y,1,1,reg1::dxdx,content(-1,0),0,reg1::set);
  pr->operate(x,y,1,1,reg1::dydy,content(-1,0),0,reg1::add);
  pr->operate(x,y,1,1,reg1::dx,content(0,-2),vektorcx,reg1::add);
  pr->operate(x,y,1,1,reg1::dy,content(0,-2),vektorcy,reg1::add);
  pr->operate(x,y,1,1,reg1::lin_com,content(1,0),h_11,reg1::add);

  //Psi'_1=H_10 Psi_0
  if ((ad!=0) || (ab!=0)) {
  pr->operate(x,y,0,1,reg1::dx,content(-ab,ad),0,reg1::add);
  pr->operate(x,y,0,1,reg1::dy,content(ad,-ab),0,reg1::add);
  pr->operate(x,y,0,1,reg1::lin_com,content(1,0),h_10,reg1::add);
  }

  //H_D3_10
  if (ad3!=0) {
  pr->operate(x,y,0,1,reg1::dxdxdy,content(2*ad3,0),0,reg1::add);
  pr->operate(x,y,0,1,reg1::dxdydy,content(0,2*ad3),0,reg1::add);

  pr->operate(x,y,0,1,reg1::dxdx,content(0,2*ad3),vektorcy,reg1::add);
  pr->operate(x,y,0,1,reg1::dydy,content(-2*ad3,0),vektorcx,reg1::add);
  pr->operate(x,y,0,1,reg1::dxdy,content(ad3,0),hd3_10_1,reg1::add);

  pr->operate(x,y,0,1,reg1::dx,content(ad3,0),hd3_10_2,reg1::add);
  pr->operate(x,y,0,1,reg1::dy,content(ad3,0),hd3_10_3,reg1::add);

  pr->operate(x,y,0,1,reg1::lin_com,content(ad3,0),hd3_10_4,reg1::add);
...
{
  content val;
  for (int i=0;i<N;i++) {val=x[i];x[i]=y[i];y[i]=val;}
}*/
...
void hamiltonian_init_graphene(reg1& region)
{
  ...
  if (dof_prototype::spin<dof_prototype::end) {  
