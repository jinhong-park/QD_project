...
int main()
{
  ...
  parameters.set("dress3",11,"eVA^3");    //dresselhaus cubic
...
*/
  ...
  parameters.set("B",1.0,"Tesla");         //magnetic field (earth's magn field ~5E-5T)
  parameters.set("theta_B",82.5/180.0,"pi");        //0->perpendicular field; 0.5->in-plane field
  ...
  parameters.set("phi_d",60.0/180.0,"pi");         //in-plane orientation with respect to crystallographic x axis
  ...
  const char* name[]={"measured-relax","measured-1e","data/Leon/measured-relax-all"};
  output_class output(2,"data/Leon/",name,"log");
  
  
  //!the outer loop
  //int configs=10;
  prec W[100][100], Wx1[100], Wx2[100];
  int pntr1=-1, pntr2;
  //for (prec p1=0;p1<configs;p1++)
  for (prec p1=0;p1<2;p1+=1)
  {
    Wx1[++pntr1]=p1;
    if (p1!=0) parameters.set("dress3",0,"eVA^3");
    //parameters.set("theta_B",0.5*(90-p1)/90,"pi");
    //parameters.set("phi_d",p1/180.0,"pi");
    //parameters.set("phi_B",-p1/180.0,"pi");
    
    //output.set_appendix("phi_d",(int) round(p1/10));      //appendix to the output files
    ...
    pntr2=-1;
    for (prec p2=1;p2<14;p2+=step*25) {
      
      Wx2[++pntr2]=p2;
      //double hwx, hwy;
      //interpolate_hw(p2, hwx, hwy);
      //set_ALHO(hwx, hwy);
      ...
      //prec g=-0.44*(1.0-(1.0-160.0/200.0)*(p2*p2/64.0));
      //parameters.set("gx",g,"-");           //g factors
      //parameters.set("gy",g,"-");
      //parameters.set("gz",g,"-");
      ...
      for (int from=0; from<from_last; from++ ) {//initial state
        ...
        for (int to=0; to<1; to++) {    //final state
          ...
          if (from==1 && to==0) W[pntr1][pntr2]=rate_sum;
  ...
  FILE* out=fopen(name[2],"w");
  for (int i=0;i<pntr2+1;i++) {
    fprintf(out,"%e ",Wx2[i]);
    for (int j=0;j<pntr1+1;j++) {
      fprintf(out,"%e ",W[j][i]);
    }
    fprintf(out,"\n");
  }
