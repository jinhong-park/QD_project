...
int main()
{
    ...
    const char* name[]={"energies","parameters"};
    output_class output(2,"data/Jin/",name,"log");
...

    {
        ...
        double lx=lv*(1+d/lv);
        double ly=lv/(1+d/lv);
        double hwx=units.hbar*units.hbar/(2*units.m_e*m_eff*lx*lx*units.nm*units.nm)/units.meV;
        double hwy=units.hbar*units.hbar/(2*units.m_e*m_eff*ly*ly*units.nm*units.nm)/units.meV;

        fprintf(output.file(1),"lbr=%.0f nm, ldress=%.0f nm, lz=%.0f nm, lx=%.1f nm, ly=%.1f nm, hwx=%.1f meV, hwy=%.1f meV\n", lbr, ldress, lz, lx, ly, hwx, hwy);
    }
    
    //!the outer loop
    for (prec p1=0;p1<180;p1+=30)
    {
