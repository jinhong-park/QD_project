...
int main()
{
    ...
    //!quantum dot setup
    parameters.set("width_z",10.0,"nm");       //width of the 2DEG
   ...
   // parameters.set("lv",32,"nm");
    ...
    for (int bp=1; bp < 51; bp++){     // b_parallel loop
    for (int wz=1; wz < 2; wz++){
    ...
    for (prec p1=89;p1<180;p1+=200)
    {
        ...
        for (prec p2=45;p2<180;p2+=200) {
            ...
            double dwz,Bpara;
            dwz = 10.0+1.0*(((double) wz)-1.0);
            Bpara = 0.0 + 0.001*(((double) bp)-1.0); 

            parameters.set("B",Bpara,"Tesla");
            ...
            double ene[J];
            for(int q=0;q<J;q++){
            ene[q]=states->read(states->sorted2un[q],"energy");

            fprintf(output.file(0),"%.14e \t",ene[q]);

            }//printout total energy
