...
class stackPETER {
  ...
  public:
    stackPETER(int l, int k, char* tag); //constructor
    ~stackPETER();                       //desctructor
    stackPETER* next;                    //pointer to next 
...
class acr_stack {
  class stackPETER* first;
  class stackPETER* last;
  int hmn_exist;
  stackPETER* give_stack(int which);
  public:
    acr_stack();
    ~acr_stack();
...
struct dof_prototype {
    int Ndof;				//number of quasi-spin degrees of freedom,
    int length;             //number of wavefunction components: length=2^Ndof
                            //length-1=maximal value of the consecutive index
    //!for graphene sigma dof is special and has to be the first!!!
    enum name {spin,sigma,Ndof_max,tau,eta,kappa};	//generic
  ...
  //enum name {spin,tau,sigma};			//skyrmion check1
  enum op {unity, paulix, pauliy, pauliz, splus, sminus};			//operator basis within a single dof
  int *indexC;							//values of the component vector (+1/-1=up/down for each of Ndof - this is an alternative indexing to a simple consecutive one)
  int index;							//consecutive index from 0 to length-1
  ...
  void add_one_tensor(op op_in, complex<double>* M_in, int Ndof_in, complex<double>* M_out); //assuming the matrix M_in is filled up to Ndof_in pseud-spin degrees of freedom, add one more degree of freedom by tensor product
  ...
  void reset_Ndof(int Ndof);                   //change the number of components
